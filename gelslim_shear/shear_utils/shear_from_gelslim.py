...
class ShearGenerator:
    ...
    def __init__(self, method: str = 'weighted',
                 channels=['u', 'v', 'div', 'curl', 'sol_u', 'sol_v', 'irr_u', 'irr_v', 'dudt', 'dvdt', 'du', 'dv'],
    ...
    def extract_flow_field(self, ref, image):
        flow = cv2.calcOpticalFlowFarneback(ref, image, None, self.Farneback_params[0], self.Farneback_params[1],
                                            self.Farneback_params[2], self.Farneback_params[3],
                                            self.Farneback_params[4], self.Farneback_params[5],
    ...
    def update_shear(self):
        ...
        if self.method == 'weighted':
            if 'u' in self.channels or 'v' in self.channels or 'div' in self.channels or 'curl' in self.channels or 'sol_u' in self.channels or 'irr_u' in self.channels or 'sol_v' in self.channels or 'irr_v' in self.channels:
                weight = self.sigmoid_weighting_function(self.displacement, self.displacement_midpoint,
